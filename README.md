...
- Зачисление на счет некоторой суммы (150.0 + номер потока)
...
Все тесты выполняются в 100 потоков и покрывают весь фунционал программы. Потоки имеют разное время запуска, что позволяет проверить блокировки
